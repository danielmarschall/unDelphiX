...
uses
  ...
  ExtCtrls, StdCtrls, Menus, DXDraws, DXClass;
...
function TfrmMain.Vox2Point(const P: T3DPoint): TPoint;
begin
  with P do
  begin
...
function TfrmMain.MakeVox(X, Y, Z: Integer): T3DPoint;
begin
  Result.X := X;
  Result.Y := Y;
  Result.Z := Z;
...
function TfrmMain.Rotate3dPoint(var FPoint: T3dPoint; FAngleX, FAngleY, FAngleZ: real): T3dPoint;
var
  x, x1, x2,
    y, y1, y2,
    z, z1, z2: real;
...
begin
  ...
  FTempDIB.DIB.Canvas.Brush.Color := clBlack;
  FTempDIB.DIB.Canvas.FillRect(FDIB.DIB.Canvas.ClipRect);
end;

function TfrmMain.Limit(Value, Limit: integer): integer;
begin
  if Value > Limit then Value := Limit;
  Result := Value;
end;

procedure TfrmMain.imgAreaMouseDown(Sender: TObject; Button: TMouseButton;
  Shift: TShiftState; X, Y: Integer);
begin
...
procedure TfrmMain.imgAreaMouseMove(Sender: TObject; Shift: TShiftState; X,
  Y: Integer);
begin
  if ssLeft in Shift then
  begin
...
procedure TfrmMain.cbAccClick(Sender: TObject);
begin
...
begin
  ...
  begin
    ...
    FTempDIB.DIB.DrawOn(FDIB.DIB.Canvas.ClipRect, FDIB.DIB.Canvas, 0, 1);
  end else
  begin
    FDIB.DIB.Canvas.Brush.Color := clBlack;
    FDIB.DIB.Canvas.FillRect(FDIB.DIB.Canvas.ClipRect);
  ...
  FDIB.DIB.DrawOn(imgArea.ClientRect, imgArea.Surface.Canvas, 0, 0);
  ...
  begin
    ...
    if doHardware in imgArea.NowOptions then
      Textout(0, 10, 'Device: Hardware')
    else
      Textout(0, 10, 'Device: Software');
